...
Writes a redacted product (FLAC + JSON sidecar) into a SEPARATE output cache dir,
...
def write_product(output_dir, capture_ts_ns, vsn, out_source, audio, samplerate,
                  subtype, source_sidecar, source_unique_id, plugin_name,
                  gate=None, noise_fill=False, max_count=None, max_bytes=None):
    """Redact `audio` and write the product to output_dir.
    ...
    final_clip = os.path.join(output_dir, final_clip_name)
    ...
    fd, tmp_clip = tempfile.mkstemp(prefix=".tmp-clip-", suffix=".flac", dir=output_dir)
    os.close(fd)
    try:
    ...
    # 3) evict oldest to fit BEFORE the new pair joins the ring.
    _evict_to_fit(output_dir, new_bytes, max_count, max_bytes)

    # 4) write the sidecar to a temp file and fsync it.
    fd, tmp_sidecar = tempfile.mkstemp(prefix=".tmp-side-", suffix=".json", dir=output_dir)
    try:
