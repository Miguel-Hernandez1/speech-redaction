...
def write_redacted_flac(audio_1d, samplerate, out_path, gate=None, noise_fill=False):
    ...
    # FLAC is integer-backed (PCM_16 here, matching main.py's WAV write), so
    # out-of-range samples -- e.g. loud noise-fill peaks -- would clip silently.
    # Clip a copy so the caller's (already-redacted) buffer is not mutated again.
    to_write = np.clip(redacted, -1.0, 1.0)
    sf.write(out_path, to_write, samplerate, format="FLAC", subtype="PCM_16")
